...
equation
  ...
  assert(uMin<=u and u<=uMax, "Limiter: signal exceeded limits!", AssertionLevel.warning);
...
(which allows the tool to avoid events wehn limits are reached). Additionally a warning is uttered when limits are exceeded.
</p>
</html>"),
