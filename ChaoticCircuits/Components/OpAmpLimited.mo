...
model OpAmpLimited "Improved operational amplifier with limitation"
  ...
  parameter ChaoticCircuits.Types.InitOpAmp initOpAmp=ChaoticCircuits.Types.InitOpAmp.Linear
...
initial equation
  if useFirstOrder then
    if initOpAmp == ChaoticCircuits.Types.InitOpAmp.UpperLimit then
      v_int=vps;
    elseif initOpAmp == ChaoticCircuits.Types.InitOpAmp.LowerLimit then
      v_int=vns;
    else
